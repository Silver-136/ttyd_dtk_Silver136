...
void kpaFireAttack(void) {
    ...
    extern f32 float_0p4_80426740;
    extern const char str_KPA_W_1_802fd2c4[], str_KPA2_W_1_802fd2cc[];
    extern const char str_KPA_R_1_802fd2d8[], str_KPA2_R_1_802fd2e0[];
    u8* mario = marioGetPtr();
    u8* work = *(u8**)(mario + 0x298);
    s32 state = *(s32*)(work + 0x13C);

#define SELECT_POSE(normalPose, twoDPose)                                      \
    (((*(u32*)((u8*)marioGetPtr() + 0x14) & 1) != 0) ? (void*)(twoDPose) :    \
                                                   (void*)(normalPose))
#define START_SOUND()                                                         \
    do {                                                                      \
        u8* soundMario = marioGetPtr();                                        \
        u8* soundWork = *(u8**)(soundMario + 0x298);                           \
        s32 soundId = *(s32*)(soundWork + 0x14C);                              \
        if ((u32)(soundId + 0x10000) == 0xFFFF) {                            \
            *(u32*)(soundWork + 0x14C) = psndSFXOn_3D(0x821,                 \
                                                       soundMario + 0x8C);     \
            psndSFXOn_3D(0x131, soundMario + 0x8C);                     \
        }                                                                      \
    } while (0)
#define STOP_SOUND()                                                          \
    do {                                                                      \
        ...
        if ((u32)(soundId + 0x10000) != 0xFFFF) {                            \
            psndSFXOff(soundId);                                               \
            *(s32*)(soundWork + 0x14C) = -1;                                  \
        }                                                                      \
    } while (0)
#define SPAWN_FIRE()                                                          \
    do {                                                                      \
        u8* fireMario = marioGetPtr();                                         \
        u8* fireWork = *(u8**)(fireMario + 0x298);                             \
        if (fireefp == NULL) {                                                 \
            s32 left;                                                         \
            f32 side;                                                         \
            u32 level;                                                        \
            ...
            f64 power;                                                        \
            s32 kind;                                                         \
            left = (*(f32*)(fireMario + 0x1AC) >= 90.0f &&                    \
                    *(f32*)(fireMario + 0x1AC) <= 270.0f);                    \
            side = left ? 1.0f : -1.0f;                                      \
            level = 0;                                                        \
            levelMario = marioGetPtr();                                       \
            if ((s32)*(u8*)(levelMario + 0x3C) == 2) {                         \
                u8* levelWork = *(u8**)(levelMario + 0x298);                   \
                level = *(u32*)(levelWork + 4);                                \
            }                                                                  \
            scale = baseScaleTbl[level];                                      \
            flagMario = marioGetPtr();                                        \
            yExtra = ((*(u32*)(flagMario + 0x14) & 1) == 0) ? 6.0f : 0.0f;    \
            power = float_0p4_80426740;                                       \
            kind = 7;                                                         \
            if (*(u32*)(fireWork + 4) == 0) power = 0.3;                       \
            else if (*(u32*)(fireWork + 4) == 1) power = 1.0;                  \
            else if (*(u32*)(fireWork + 4) == 2) power = 1.4;                  \
            else if (*(u32*)(fireWork + 4) == 3) { power = 1.5; kind = 0; }    \
            fireefp = effGonbabaBreathEntry(                                  \
                *(f32*)(fireMario + 0x8C) + 40.0f * side * scale,             \
                *(f32*)(fireMario + 0x90) + 20.0f * scale + yExtra,           \
                *(f32*)(fireMario + 0x94), power, kind, 0);                   \
            {                                                                 \
                s32 targetLeft = (*(f32*)(fireMario + 0x1B0) >= 90.0f &&       \
                                  *(f32*)(fireMario + 0x1B0) <= 270.0f);        \
                u8* effectWork = *(u8**)((u8*)fireefp + 0xC);                 \
                *(f32*)(effectWork + 0x48) = targetLeft ? 180.0f : 0.0f;       \
                if (*(void**)(effectWork + 0x4C) != NULL) {                   \
                    u8* child = *(u8**)((u8*)*(void**)(effectWork + 0x4C) + 0xC);\
                    *(f32*)(child + 0x48) = targetLeft ? 180.0f : 0.0f;         \
                }                                                             \
            }                                                                 \
        }                                                                      \
    } while (0)
    ...
    switch (state) {
        ...
        case 0x14:
            marioChgPose(SELECT_POSE("KPA_A_1A", "KPA2_A_1A"));
            *(s32*)(work + 0x140) = 6;
            *(s32*)(work + 0x13C) = 0x15;
            break;
        case 0x15:
            if (--*(s32*)(work + 0x140) <= 0) {
                marioChgPose(SELECT_POSE("KPA_A_1B", "KPA2_A_1B"));
                *(s32*)(work + 0x140) = 8;
                *(s32*)(work + 0x13C) = 0x16;
                START_SOUND();
            }
        ...
        case 0x16:
            ...
            if (--*(s32*)(work + 0x140) <= 0) {
                *(s32*)(work + 0x140) = sysMsec2Frame(400);
                marioChgPose(SELECT_POSE("KPA_A_1C", "KPA2_A_1C"));
                *(s32*)(work + 0x13C) = 0x17;
            }
            break;
        case 0x17:
            SPAWN_FIRE();
            if (--*(s32*)(work + 0x140) <= 0 &&
                ((*(u16*)(mario + 0x24A) & 0x200) == 0) &&
                (*(s32*)(work + 0x144) == 0)) {
                marioChgPose(SELECT_POSE("KPA_A_1D", "KPA2_A_1D"));
                *(s32*)(work + 0x13C) = 0x18;
                *(s32*)(work + 0x140) = 6;
                STOP_SOUND();
            }
        ...
        case 0x18:
            if (--*(s32*)(work + 0x140) <= 0) {
                *(s32*)(work + 0x13C) = 0;
                marioChgPose(SELECT_POSE("KPA_S_1", "KPA2_S_1"));
            }
            break;
    }
    ...
    switch (state) {
        ...
        case 0x2C:
            if (--*(s32*)(work + 0x140) <= 0) {
                *(s32*)(work + 0x13C) = 0;
                if (*(u16*)(mario + 0x2E) == 1) {
                    marioChgPose(SELECT_POSE(str_KPA_R_1_802fd2d8, str_KPA2_R_1_802fd2e0));
                } else if (*(u16*)(mario + 0x2E) == 0) {
                    marioChgPose(SELECT_POSE("KPA_S_1", "KPA2_S_1"));
                } else {
                    marioChgPose(SELECT_POSE(str_KPA_W_1_802fd2c4, str_KPA2_W_1_802fd2cc));
                }
            }
            break;
    }
...
void kpaFireMain(void) {
    ...
    *(void**)(*(u8**)(mario + 0x298) + 0x18) = NULL;
    if (fireefp != NULL) {
        ...
        if (*(f32*)(mario + 0x1AC) >= 90.0f &&
            *(f32*)(mario + 0x1AC) <= 270.0f) {
            left = 1;
        }
        if (left) {
            side = 1.0f;
        ...
        mario = marioGetPtr();
        if (*(u8*)(mario + 0x3C) == 2) {
            level = *(u32*)(*(u8**)(mario + 0x298) + 4);
        }
        scale = baseScaleTbl[level];
        x = 40.0f * side * scale + *(f32*)(mario + 0x8C);
        y = *(f32*)(mario + 0x90) + 20.0f * scale;
        if ((*(u32*)((u8*)marioGetPtr() + 0x14) & 1) == 0) {
            y += 6.0f;
        }
        z = *(f32*)(mario + 0x94);
        *(f32*)(*(u8**)((u8*)fireefp + 0xC) + 4) = x;
        *(f32*)(*(u8**)((u8*)fireefp + 0xC) + 8) = y;
        *(f32*)(*(u8**)((u8*)fireefp + 0xC) + 0xC) = z;
        if (*(void**)(*(u8**)((u8*)fireefp + 0xC) + 0x4C) != NULL) {
            *(f32*)(*(u8**)(*(u8**)(*(u8**)((u8*)fireefp + 0xC) + 0x4C) + 0xC) + 4) = x;
            *(f32*)(*(u8**)(*(u8**)(*(u8**)((u8*)fireefp + 0xC) + 0x4C) + 0xC) + 8) = y;
            *(f32*)(*(u8**)(*(u8**)(*(u8**)((u8*)fireefp + 0xC) + 0x4C) + 0xC) + 0xC) = z;
        }
        ...
        if (level != 0) {
            ...
            camGetPtr(4);
            work = *(u8**)(effect + 0xC);
            rootScale = *(f32*)(work + 0x40);
            PSMTXTrans(trans, *(f32*)(work + 4), *(f32*)(work + 8), *(f32*)(work + 0xC));
            PSMTXRotRad(rot, 'y', 0.017453292f *
                        (*(f32*)(work + 0x48) - *(f32*)((u8*)camGetPtr(4) + 0x114)));
            if (*(f32*)(work + 0x48) > 90.0f &&
                *(f32*)(work + 0x48) < 270.0f) {
                PSMTXScale(scaleMtx, rootScale, rootScale, -rootScale);
            } else {
                PSMTXScale(scaleMtx, rootScale, rootScale, rootScale);
            }
            PSMTXConcat(trans, rot, trans);
            PSMTXConcat(trans, scaleMtx, rootMtx);
            for (i = 1, work += 0x50; i < *(s32*)(effect + 8); i++, work += 0x50) {
                if (((i - 1) & 3) == 0 && *(s32*)(work + 0x44) == 0 &&
                    *(s32*)(work + 0x1C) > 16) {
                    particleScale = 7.0f * *(f32*)(work + 0x40);
                    if (particleScale * rootScale >= 7.0f) {
                        ...
                        PSMTXConcat(rootMtx, trans, trans);
                        origin.x = *(f32*)(vec3_802fd238 + 0);
                        origin.y = *(f32*)(vec3_802fd238 + 4);
                        origin.z = *(f32*)(vec3_802fd238 + 8);
                        PSMTXMultVec(trans, &origin, &position);
                        *(void**)(*(u8**)(mario + 0x298) + 0x18) =
                            kpaFlameHitCheck(position.x, position.y, position.z, particleScale * rootScale);
                        if (*(void**)(*(u8**)(mario + 0x298) + 0x18) != NULL) {
                            break;
                        }
                    }
                }
    ...
    mario = marioGetPtr();
    if (((*(u32*)mario & 1) == 0) ||
        !((*(s32*)(*(u8**)(mario + 0x298) + 0x13C) >= 0x14 &&
           *(s32*)(*(u8**)(mario + 0x298) + 0x13C) <= 0x17) ||
          (*(s32*)(*(u8**)(mario + 0x298) + 0x13C) >= 0x28 &&
           *(s32*)(*(u8**)(mario + 0x298) + 0x13C) <= 0x2B))) {
        if (fireefp != NULL) {
            effSoftDelete(fireefp);
            fireefp = NULL;
        }
