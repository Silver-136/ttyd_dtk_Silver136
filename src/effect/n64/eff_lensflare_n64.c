...
#define GXSetTevColorIn(a, b, c, d, e) GXSetTevColorIn(a, b, 12, d, e)
#define GXSetTevAlphaIn(a, b, c, d, e) GXSetTevAlphaIn(a, 7, c, d, e)
#define effSetVtxDescN64(a) (GXSetTevColor(1, &color), effSetVtxDescN64(a))
...
#undef GXSetTevColorIn
#undef GXSetTevAlphaIn
#define GXSetTevColor(a, b) (GXSetTevColor(a, b), GXSetNumTevStages(1), GXSetTevOrder(0, 0, 0, 0xFF), GXSetTevColorOp(0, 0, 0, 0, 1, 0), GXSetTevAlphaOp(0, 0, 0, 0, 1, 0), GXSetTevColorIn(0, 15, 15, 15, 2), GXSetTevAlphaIn(0, 0, 1, 4, 7))
#undef GXSetTevColor
    for (i = 1; i < *(s32*)((u8*)effect + 8); i++, p += 0x34, angle += 10, yrot += 35) {
        f32 sc = *(f32*)(p + 0x28);
        if (sc != 0.0f) {
            color = 0xFFFFFF00 | (u8)((*(s32*)(p + 0x14) * alpha) >> 8);
            ...
            PSMTXRotRad(r, 0x7A, *(f32*)(p + 0x20) * 0.0174533f);
            PSMTXTrans(m, *(f32*)(p + 0x2C), 0.0f, 0.0f);
            PSMTXConcat(r, m, m);
            PSMTXScale(s, sc, sc, sc);
            PSMTXConcat(m, s, m);
            PSMTXConcat(base, m, m);
            GXLoadPosMtxImm(m, 0);
            GXBegin(0x90, 0, 6);
            tri2(12, 13, 14, 0, 12, 14, 15, 0);
            PSMTXRotRad(r, 0x7A, *(f32*)(p + 0x20) * 0.0174533f);
            PSMTXTrans(m, *(f32*)(p + 0x2C) + float_5_80425978, 0.0f, 0.0f);
            PSMTXConcat(r, m, m);
            sc = 2.3f - sc;
            PSMTXScale(s, sc, sc, sc);
            PSMTXConcat(m, s, m);
            ...
            GXLoadPosMtxImm(m, 0);
            GXBegin(0x90, 0, 6);
            tri2(8, 9, 10, 0, 8, 10, 11, 0);
            color = 0xFFFFFF00 | (u8)((*(s32*)(p + 0x14) * alpha) >> 9);
            GXSetTevColor(1, &color);
            color2 = 0x78787800;
