...
void effRotHammerFailDisp(s32 cameraId, void* effect) {
    ...
    extern u32 dat_80425f48;
    extern f32 float_32_80425f60;
    u8 texObj[0x20];
    f32 trans[3][4], rot[3][4], mtx[3][4], scale[3][4];
    u8* work = *(u8**)((s32)effect + 0xC);
    void* camera = camGetPtr(cameraId);
    s32 frame = (s32)*(f32*)(work + 0x30);
    u32 color;
    s32 i;

    GXSetNumChans(0);GXSetNumTexGens(2);GXSetTexCoordGen2(0,1,4,0x1E,0,0x7D);GXSetTexCoordGen2(1,1,4,0x21,0,0x7D);effGetTexObjN64(0xD,texObj);GXLoadTexObj(texObj,0);GXLoadTexObj(texObj,1);GXSetNumTevStages(3);GXSetTevOrder(0,0,0,0xFF);GXSetTevColorOp(0,0,0,0,1,0);GXSetTevAlphaOp(0,0,0,0,1,0);GXSetTevColorIn(0,0,0,0,0);GXSetTevAlphaIn(0,0,0,0,4);GXSetTevOrder(1,1,1,0xFF);GXSetTevColorOp(1,0,0,0,1,0);GXSetTevAlphaOp(1,0,0,0,1,0);GXSetTevColorIn(1,0,0,0,0);GXSetTevAlphaIn(1,6,7,2,7);GXSetTevOrder(2,0xFF,0xFF,0xFF);GXSetTevColorOp(2,0,0,0,1,0);GXSetTevAlphaOp(2,0,0,0,1,0);GXSetTevColorIn(2,0,0,0,2);GXSetTevAlphaIn(2,0,6,1,7);GXSetCullMode(0);effSetVtxDescN64((void*)0x803A80C0);
    PSMTXTrans(trans,*(f32*)(work+4),*(f32*)(work+8),*(f32*)(work+0xC));PSMTXRotRad(rot,0x79,float_deg2rad_80425f50*-*(f32*)((s32)camGetPtr(4)+0x114));PSMTXConcat(trans,rot,trans);PSMTXConcat((void*)((s32)camera+0x11C),trans,mtx);
    color = dat_80425f48;GXSetTevColor(1,&color);color=(u32)(float_256_80425f4c*(*(f32*)(work+0x30)-(f32)frame));GXSetTevColor(2,&color);
    PSMTXScale(scale,float_0p0044643_80425f54,float_0p03125_80425f58,float_0_80425f5c);
    PSMTXTrans(trans, (f32)(frame * 0x20), float_0_80425f5c, float_0_80425f5c);
    PSMTXConcat(scale, trans, trans);GXLoadTexMtxImm(trans,0x1E,1);
    PSMTXScale(scale,float_0p0044643_80425f54,float_0p03125_80425f58,float_0_80425f5c);
    if (frame == 6) {
        PSMTXTrans(trans, -float_32_80425f60, float_32_80425f60, float_0_80425f5c);
    } else {
        PSMTXTrans(trans, (f32)((frame + 1) * 0x20), float_0_80425f5c, float_0_80425f5c);
    }
    PSMTXConcat(scale, trans, trans);GXLoadTexMtxImm(trans,0x21,1);
    for (i = 1; i < *(s32*)((s32)effect + 8); i++, work += 0x34) {
        PSMTXTrans(trans, *(f32*)(work + 0x38), *(f32*)(work + 0x3C), *(f32*)(work + 0x40));
        PSMTXConcat(mtx, trans, trans);GXLoadPosMtxImm(trans,0);GXSetCurrentMtx(0);GXBegin(0x90,0,6);tri2(0,1,2,0,0,2,3,0);
    }
}

u8 size24x24_tex32x32_vtx[] = {
    0xFF, 0xF4, 0xFF, 0xF4, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x0C,
    0xFF, 0xF4, 0x00, 0x00, 0xFC, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0xFF, 0x00, 0x0C, 0x00, 0x0C,
    0x00, 0x00, 0xFC, 0x00, 0x04, 0x00, 0x00, 0x00,
    0x00, 0xFF, 0xFF, 0xF4, 0x00, 0x0C, 0x00, 0x00,
    0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0xFF,
};

const f32 vec3_802fbe70[3] = { 0.0f, 0.0f, 0.0f };
const char str_RotHammerFailN64_802fbe88[] = "RotHammerFailN64";
