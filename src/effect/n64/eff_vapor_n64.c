...
void effVaporDisp(s32 cameraId, void* effect)  {
    ...
    extern const s8 dx_data[];
    extern const u8 scale_data[];
    extern const u8 seq_data[];
    extern const u8 rz_data[];
    ...
    c=(*(s32*)(w+0x18)<<24)|(*(s32*)(w+0x1C)<<16)|(*(s32*)(w+0x20)<<8)|((s32)(0.5f*(f32)*(s32*)(w+0x24))&0xFF);
    GXSetTevColor(1,&c);
    if (type == 0 || type == 2) {
        GXSetNumChans(0);
        GXSetNumTevStages(3);
        ...
        GXSetTevAlphaOp(2,0,0,0,1,0);
        GXSetTevColorIn(2,3,2,0,0);
        GXSetTevAlphaIn(2,0,0,0,0);
        effGetTexObjN64(0x6A,texObj);
    }
    ...
    index=type*30+frame;
    PSMTXTrans(t,(f32)dx_data[index],0.0f,0.0f);
    PSMTXRotRad(r,0x7A,-0.017453292f*(f32)rz_data[index]);
    anim=0.01f*(f32)scale_data[index];
    PSMTXScale(s,anim,anim,anim);
    PSMTXConcat(t,r,t);
    PSMTXConcat(t,s,t);
    PSMTXConcat(base,t,t);
    GXLoadPosMtxImm(t,0);
    GXSetCurrentMtx(0);
    anim=5.0f*(f32)seq_data[index]*0.00390625f;
    cell=(s32)anim;
    frac=(s32)(256.0f*(anim-(f32)cell));
    ...
    if(cell<5) {
        if (cell < 4) {
            PSMTXScale(s,0.032258f,0.007874f,0.0f);
            GXLoadTexMtxImm(s,0x21,1);
        } else {
            PSMTXScale(s,0.0f,0.0f,0.0f);
            GXLoadTexMtxImm(s,0x21,1);
        }
        c=(*(s32*)(w+0x28)<<24)|(*(s32*)(w+0x2C)<<16)|(*(s32*)(w+0x30)<<8)|(frac&0xFF);
        GXSetTevColor(2,&c);
        GXBegin(0x90,0,6);
        tri2(cell*4,cell*4+1,cell*4+2,0,cell*4,cell*4+2,cell*4+3);
    }
}
