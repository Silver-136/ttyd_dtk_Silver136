...
u8 effHolographyDisp(s32 cameraId, void* effect) {
    ...
    GXSetNumChans(0);GXSetNumTevStages(3);GXSetTevOrder(0,0,0,0xFF);GXSetTevColorOp(0,0,0,0,1,0);GXSetTevAlphaOp(0,0,0,0,1,0);GXSetTevColorIn(0,15,15,15,8);GXSetTevAlphaIn(0,7,7,7,4);GXSetTevOrder(1,1,1,0xFF);GXSetTevColorOp(1,0,0,0,1,0);GXSetTevAlphaOp(1,0,0,0,1,0);GXSetTevColorIn(1,8,0,5,15);GXSetTevAlphaIn(1,4,0,1,7);GXSetTevOrder(2,1,1,0xFF);GXSetTevColorOp(2,0,0,0,1,0);GXSetTevAlphaOp(2,0,0,0,1,0);GXSetTevColorIn(2,15,15,15,0);GXSetTevAlphaIn(2,7,0,1,7);
    effGetTexObjN64(0x93,tex);GXLoadTexObj(tex,0);effGetTexObjN64(0x94,tex);GXLoadTexObj(tex,1);GXSetNumTexGens(2);GXSetTexCoordGen2(0,1,4,0x1E,0,0x7D);GXSetTexCoordGen2(1,1,4,0x1E,0,0x7D);desc=0x36-frame;if(desc<0)desc=0;effSetVtxDescN64(ach_01_v+desc*0x134);color=unk_804296f0;((u8*)&color)[3]=xlu_data[frame%16];GXSetTevColor(2,&color);PSMTXScale(s,0.03125f,0.03125f,0.0f);GXLoadTexMtxImm(s,0x1E,1);
    for(i=0;i<10;i++){a=(s32)(2.0f*(f32)*(s32*)(w+0x24)*(1.0f-0.1f*(f32)i));if(a>255)a=255;color=((u8)*(s32*)(w+0x18)<<24)|((u8)*(s32*)(w+0x1C)<<16)|((u8)*(s32*)(w+0x20)<<8)|(u8)a;GXSetTevColor(1,&color);GXBegin(0x90,0,6);switch(i){case 0:tri2(0,1,2,0,0,2,3,0);break;case 1:tri2(1,4,5,0,1,5,2,0);break;case 2:tri2(4,6,7,0,4,7,5,0);break;case 3:tri2(6,8,9,0,6,9,7,0);break;case 4:tri2(8,10,11,0,8,11,9,0);break;case 5:tri2(10,12,13,0,10,13,11,0);break;case 6:tri2(12,14,15,0,12,15,13,0);break;case 7:tri2(14,16,17,0,14,17,15,0);break;case 8:tri2(16,18,19,0,16,19,17,0);break;case 9:tri2(18,20,21,0,18,21,19,0);break;}}
    return 0;
}
